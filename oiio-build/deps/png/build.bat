...
set LIB_PATH=%USERPROFILE%\Documents\png

set LIB_SRC_PATH=%LIB_PATH%\lpng1637
set BUILD_PATH=%LIB_PATH%\build
set INSTALL_PATH=%LIB_PATH%\install

REM - TODO: add each library (automatically?) to build options, and use a simple command with a '%BUILD_OPTIONS%' parameter
set ZLIB_PATH=%LIB_PATH%\deps\zlib\%ARCH%\%BUILD_TYPE%
